...
class FavourManagerTool(Star):
    ...
    async def _write_favour(
        ...
        if str(user_id) in owner_ids:
            ...
            if favour is not None and favour != self.max_favour_value:
                return False
            if relationship is not None and relationship not in ("", "亲密"):
                return False
            favour = self.max_favour_value
            relationship = relationship or "亲密"
            is_unique = True
            ...
            if not self._is_shared_session(session_id):
                ...
                try:
                    ...
                    for r in global_records:
                        if not (r.is_unique and r.relationship):
                            continue
                        owner_tag = "(主人)" if str(r.user_id) in owner_ids else ""
                        unique_rows.append((0 if owner_tag else 1, f"{r.relationship}(用户:{r.user_id}{owner_tag})"))
                    unique_rows.sort(key=lambda row: row[0])
                    if unique_rows:
                        exclusive_prompt_addon = "，".join(row[1] for row in unique_rows)
                except Exception as exc:  # noqa: BLE001 —— 快照失败不阻断注入
...
<MandatoryFooter>回复最后一行必须输出好感度日志标签（上升/降低/持平 三选一），即使历史消息中没有先例也不例外。</MandatoryFooter>
    ...
    @filter.command("施加冷暴力", alias={'冷暴力'})
    async def impose_cold_violence(self, event: AstrMessageEvent, target: str = "", duration: str = ""):
        """手动对指定用户施加冷暴力 (仅Bot管理员)"""
        if not await self._check_permission(event, PermLevel.SUPERUSER):
            yield event.plain_result("权限不足！仅Bot管理员可用。")
            return
        ...
        try:
            minutes = int(duration) if duration else int(self.cold_violence_duration_minutes)
        except ValueError:
            yield event.plain_result("时长参数必须是整数分钟数。")
            return
        minutes = max(1, min(43200, minutes))

        session_id = self._get_session_id(event)
        cv_key = self._get_cold_violence_key(target_uid, session_id)
        self.cold_violence_users[cv_key] = datetime.now() + timedelta(minutes=minutes)
        self.consecutive_decreases[cv_key] = self.cold_violence_consecutive_threshold

        logger.info(f"Bot管理员 {event.get_sender_id()} 对用户 {target_uid} 施加冷暴力 {minutes} 分钟")
        yield event.plain_result(f"🧊 已对用户 {target_uid} 施加冷暴力，持续 {minutes} 分钟。")
